...
def run_mpbit(df, holdout, covs, covs_max_list, threshold, tradeoff_param = 0.1):
    ...
    total_match_num += match_indicator.sum()
    ...
    while len(pos_drops)>0: # we still have sets to drop
        ...
        #print("match_indicator_for_all: ", (best_res[-3]).tolist().count(True))
        #print("match_indicator: ",(best_res[-2]).tolist().count(True))
        ...
        print("level", 18 - len(set_dropped))
        total_match_num += best_res[-2].sum()
        print(best_res[-2].sum())
...
def get_catt(df, matching_res, num_t):
    ...
    true_catt_init = df['true_effect'].tolist()[15000:]
...
if __name__ == '__main__':
    ...
    res = run_mpbit(df, holdout, range(18), [2]*18, 0.3,tradeoff_param = 0.1)
    x, y = get_catt(df, res[0], df.shape[0] * 0.5)
    print(len(x))
    print(len(y))
    print(MSE(x,y))
