...
def merge_results(matching_res, catt_res, num_t):
    for match_res_level in matching_res:
        for match_grp in match_res_level:
            ...
            for i in range(len(match_grp[2])):
                matched_index = match_grp[2][i]
...
if __name__ == '__main__':
    ...
    for i in range(imputation_num):
        ...
        res = run_mpbit(df, holdout, range(15), [2]*15, 0, tradeoff_param = 0.1)

        merge_results(res[0], catt_res, int(df.shape[0] * 0.5))

    avg_catt_res = get_avg_imputation(catt_res)
    print(avg_catt_res)
    pickle.dump(avg_catt_res, open("res/DAME-imputation", 'wb'))
