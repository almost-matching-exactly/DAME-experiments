...
def run_mpbit(df, holdout, miss, covs, covs_max_list, threshold, tradeoff_param = 0.1):
    total_units_num = df.shape[0]
    print(total_units_num)
    dropped = set()
    covs = list(covs)
    ...
    match_indicator_for_all, index = match_mp(df, miss, all_covs, covs_max_list) 
    ...
    while len(pos_drops)>0: # we still have sets to drop
        ...
        if df[(df['treated'] == 0) & (df['matched'] == 0)].shape[0]==0 or df[(df['treated'] == 1) & (df['matched'] == 0)].shape[0]==0: 
            print('no more matches')
            break
        
        
        # added to put theshold on number of units matched
        if sum(nb_match_units) >= 1300: 
        ...
        for s in pos_drops:
            ...
            match_indicator_for_all, index = match_mp(df, miss, cur_covs_no_s,
        ...
        """
        if (init_score < 0 and cur_score <= init_score * 1.20) or (init_score >= 0 and cur_score <= init_score * 0.80):
            print("early stop")
            break
        """
        ...
        #print("BF ", BFs)
        #print("num of cov ", 15 - len(set_dropped))
...
def get_catt(df, matching_res, num_t):
    estimated_catt_init = [None] * int(len(df) - num_t)
    true_catt_init = df['true_effect'].tolist()[num_t:]
    ...
    for matching_level in matching_res:
        ...
        for matching_group in matching_level:
            for idx in matching_group[2]:
                if int(idx) >= num_t:
                    estimated_catt_init[int(idx) - int(num_t)] = matching_group[0]
...
def get_total_matched(matching_res):
...
if __name__ == '__main__':
    ...
    res = run_mpbit(df, holdout, miss, range(df.shape[1] - 5), [2]*(df.shape[1] - 5), 0.1,tradeoff_param = 0.1)

    x,y = get_catt(df, res[0], int(df.shape[0] * 0.5))

    print("MSE: " + str(MSE(x,y)))
    get_total_matched(res[0])

    
    pickle.dump(x, open("res/DAME-x-miss", 'wb'))
    pickle.dump(y, open("res/DAME-y-miss", 'wb'))
